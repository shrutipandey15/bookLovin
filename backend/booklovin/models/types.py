...
class PostService(Protocol):
    ...
    async def get_one(self, db: Any, post_id: str) -> Post | UserError: ...
    async def get_all(self, db: Any, start: int, end: int) -> List[Post] | UserError: ...
...
class UserService(Protocol):
    async def get(self, db: Any, email: str) -> User | UserError: ...
    async def create(self, db: Any, user: User) -> UserError: ...
