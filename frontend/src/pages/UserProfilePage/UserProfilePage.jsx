...
import { User, BookOpen, MessageSquare, Loader } from 'lucide-react';
import PostCard from '@components/PostCard';

const ShelfBookCard = ({ shelfItem }) => (
    <div className="text-center">
        <img src={shelfItem.book.cover_image_url} alt={shelfItem.book.title} className="w-full h-48 object-cover rounded-md shadow-lg transition-transform hover:scale-105" />
  ...
  const ShelvesContent = () => {
    ...
    return (
        <div className="space-y-10">
            {reading.length > 0 && <div><h3 className="text-xl font-bold text-primary mb-4">Currently Reading</h3><div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4">{reading.map(item => <ShelfBookCard key={item.id} shelfItem={item} />)}</div></div>}
            {read.length > 0 && <div><h3 className="text-xl font-bold text-primary mb-4">Read</h3><div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4">{read.map(item => <ShelfBookCard key={item.id} shelfItem={item} />)}</div></div>}
            {wantToRead.length > 0 && <div><h3 className="text-xl font-bold text-primary mb-4">Want to Read</h3><div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4">{wantToRead.map(item => <ShelfBookCard key={item.id} shelfItem={item} />)}</div></div>}
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 bg-background font-body">
      ...
      <div className="mb-8 flex justify-center border-b border-secondary">
        ...
        <button onClick={() => setActiveTab('reflections')} className={`px-6 py-3 text-sm font-medium transition-colors flex items-center gap-2 ${activeTab === 'reflections' ? 'border-b-2 border-primary text-primary' : 'text-secondary hover:text-primary'}`}>
      ...
      <div>
        ...
        {activeTab === 'reflections' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {posts.length > 0 ? posts.map(p => <PostCard key={p.uid} post={p} />) : <p className="col-span-full text-center text-secondary py-10">No reflections shared yet.</p>}
          </div>
        )}
      </div>
    </div>
