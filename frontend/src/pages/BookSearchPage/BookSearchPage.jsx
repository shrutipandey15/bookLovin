...
const BookResultCard = ({ book }) => {
    ...
    const handleAddToShelf = (status) => {
        ...
        // Ideally, you'd show a success toast here
        alert(`Added "${book.title}" to your "${status.replace('_', ' ')}" shelf!`);
    };

    return (
        <div className="flex gap-4 rounded-lg border border-secondary p-4 bg-background/50">
            ...
            <div className="flex-1">
                ...
                <p className="text-sm text-text-primary line-clamp-2">{book.description}</p>
                 <div className="mt-3 flex gap-2">
                    <button onClick={() => handleAddToShelf('want_to_read')} className="text-xs px-3 py-1 rounded-full bg-blue-500/20 text-blue-300 hover:bg-blue-500/40">Want to Read</button>
                    <button onClick={() => handleAddToShelf('reading')} className="text-xs px-3 py-1 rounded-full bg-yellow-500/20 text-yellow-300 hover:bg-yellow-500/40">Reading</button>
                    <button onClick={() => handleAddToShelf('read')} className="text-xs px-3 py-1 rounded-full bg-green-500/20 text-green-300 hover:bg-green-500/40">Read</button>
                </div>
            </div>
        </div>
    )
      ...
      <form onSubmit={handleSearch} className="flex gap-2 mb-8 sticky top-24 z-10">
        <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-lg border border-secondary bg-background/80 backdrop-blur-sm py-3 pl-12 pr-10 text-lg text-text-primary focus:outline-none focus:ring-2 focus:ring-primary"
            />
