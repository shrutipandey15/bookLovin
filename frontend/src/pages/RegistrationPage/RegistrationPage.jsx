...
const RegistrationPage = () => {
  ...
  const validatePassword = (p) => {
    if (p.length < PASSWORD_MIN_LENGTH || p.length > PASSWORD_MAX_LENGTH) {
      return {
        valid: false,
        message: `Rune must be ${PASSWORD_MIN_LENGTH}-${PASSWORD_MAX_LENGTH} characters.`,
      };
    }
    ...
    if (!hasUpper || !hasLower || !hasNumber || !hasSpecial) {
      return {
        valid: false,
        message:
          "Rune needs uppercase, lowercase, number, and special characters.",
      };
    }
  ...
  const validateForm = useCallback(() => {
    ...
    if (username && !validateUsername(username))
      errors.username =
        "Pen Name must be 3-30 characters (letters, numbers, _, -).";
    if (email && !validateEmail(email))
      errors.email = "Please provide a valid Ravenmail address.";
    if (password) {
      const pwValidation = validatePassword(password);
      if (!pwValidation.valid) errors.password = pwValidation.message;
    }
    if (confirmPassword && password !== confirmPassword)
      errors.confirmPassword = "Runes do not match.";
    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4 font-body">
      <div className="w-full max-w-md rounded-2xl bg-card p-8 shadow-2xl">
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-4 text-3xl font-bold text-text-primary font-heading">
            Welcome to BookLovin'
          </h1>
          <p className="mt-2 text-text-secondary">
            Your enchanted reading sanctuary awaits
          </p>
        </div>
        ...
        <div className="mb-6 grid grid-cols-2 gap-2 rounded-full bg-background p-1">
          <Link
            to="/login"
            className="rounded-full py-2 text-center font-semibold text-text-secondary hover:bg-black/5"
          >
            Sign In
          </Link>
          <Link
            to="/register"
            className="rounded-full bg-primary py-2 text-center font-semibold text-text-contrast shadow"
          >
            Join Us
          </Link>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4" noValidate>
          ...
          {password && (
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs font-medium min-w-[70px] text-right text-secondary">
          ...
          <div>
            ...
            <input
              ...
              required
              disabled={loading || isBlocked}
              placeholder="Confirm your secret passage..."
              className="mt-1 w-full rounded-lg border border-border-color bg-background px-4 py-2 text-text-primary transition-colors focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary disabled:cursor-not-allowed disabled:opacity-50"
            />
